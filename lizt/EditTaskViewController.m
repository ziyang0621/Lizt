...
-(void)setUpTextFields {
    ...
    _taskDueTimeTextField.text = [formatter stringFromDate:_task.taskDueTime];
    ...
    if (_task.taskRemindTime) {
        _taskRemindTimeTextField.text = [formatter stringFromDate:_task.taskRemindTime];
    }
    else {
    ...
    _taskNotesTextView.text = [[NSString alloc] initWithData:_task.taskNotes encoding:NSUTF8StringEncoding];
    if (!_taskNotesTextView.text.length) {
        _taskNotesTextView.text = @"Notes...";
        _taskNotesTextView.textColor = [UIColor colorWithHex:0xC7C7CC];
    }
    
    if (_task.taskRemindTime) {
        [_remindTimeSwitch setOn:YES];
        _taskRemindTimeTextField.hidden = NO;
    }
    else {
        [_remindTimeSwitch setOn:NO];
...
- (IBAction)save:(id)sender {
    ...
    if ([trimmedString isEqualToString:@""]) {
        ...
        [_delegate controller:self didUpdateItemWithName:trimmedString andDueTime: dueTime andRemindTimeOn: _remindTimeSwitch.on andRemindTime: remindTime andNotes:  [_taskNotesTextView.text dataUsingEncoding:NSUTF8StringEncoding] andOriginalTask:_task];
